...
class Stream(Generic[T], metaclass=abc.ABCMeta):
    ...
    Conceptually, endless stream of results.
    In fact, after some "IOk" results it produces (normally the same over and over again) `IErr` results.
    """
    ...
    def collect(self) -> IResult[List[T]]:
        """
        Collects results from this stream.
